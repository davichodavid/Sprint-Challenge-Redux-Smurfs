...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
    ...
    Store holds all the data for the app. Actions are triggered by events that then prompt the reducers to do something and create another state that then replaces the current state.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

    Application state is good for scalability, should be used in large products. Component State can be used along with App State to handle smaller changes in data like form inputs.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

    Redux-thunk is middleware that intercepts actions and allows us to use axios to work with server data.
